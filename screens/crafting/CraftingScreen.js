...
function CraftingScreen({ navigation }) {
    return (
      <ScrollView>
        ...
        <Text>If an item cannot be crafted (colored in light grey or craft button missing completely), it is because of one or more of the following reasons:</Text>
        <Text>The player has insufficient resources in their inventory or in the forge. This is accompanied with a message "You don't have the necessary resources to craft this" when pressing the craft button.</Text>

        <Text>The player does not know the crafting recipe. In this case there is either no craft button at all or it is replaced by a perk button. On the recipe list, next to the item, a grey lock can also be seen, indicating that the recipe is locked.</Text>
        ...
        <Text>The workstation (a campfire, a forge or a chemistry station) doesn't have fuel. This is accompanied by a message "You don't have the required fuel to craft this" when pressing the craft button.</Text>

        <Text>The workstation (a forge or a campfire) doesn't have correct appliances. This is accompanied by a message "You don't have the required tools to craft this" when pressing the craft button.</Text>


        <Text>For more Info Check out the Crafting Mechanics Info</Text>

        <Button title="Crafting Mechanics" onPress={() => navigation.navigate('CraftingSystemScreen')} />
      </ScrollView>
    );
  }
